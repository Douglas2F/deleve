...
export default function SleepDialog({open,onClose,onSaved,onDeletedToday,onWeekChanged,initialEntry}:SleepDialogProps){
 ...
 useEffect(()=>{
  ...
  setError("");setWeekError("");setConfirmingDelete(false);setWeek(null);setSelectedDate("");setLoading(true);
  fetch("/api/health/sleep/week",{signal:controller.signal})
   .then(async response=>response.ok?response.json():Promise.reject())
   .then((body:SleepWeek)=>{if(controller.signal.aborted)return;setWeek(body);onWeekChanged(body);const selected=body.days.find(day=>day.isToday);if(selected)selectDay(selected)})
   .catch(()=>{if(!controller.signal.aborted)setWeekError("Não foi possível carregar esta semana.")})
 ...
 async function save(event:FormEvent){
  event.preventDefault();if(mutation.current||loading||!selectedDay)return;mutation.current=true;setSaving(true);setError("");
  try{
   ...
   const refreshed=await readWeek();
   onWeekChanged(refreshed);
 ...
 async function readWeek(value?:string):Promise<SleepWeek>{
  const response=await fetch(`/api/health/sleep/week${value?`?date=${encodeURIComponent(value)}`:""}`);
  if(!response.ok)throw new Error("Não foi possível carregar o resumo. Selecione a data novamente.");
 ...
 async function chooseDate(value:string){
  ...
  setSelectedDate(value);setWeek(null);setLoading(true);setError("");setWeekError("");setConfirmingDelete(false);
  try{
   const body=await readWeek(value);
   if(requestId!==dateRequest.current)return;
   setWeek(body);
   const day=body.days.find(day=>day.date===value);if(day)selectDay(day);
  }catch(caught){if(requestId===dateRequest.current)setWeekError(caught instanceof Error?caught.message:"Não foi possível carregar o sono.")}
  finally{if(requestId===dateRequest.current)setLoading(false)}
 }

 async function removeSelectedSleep(){
  if(!selectedDate||mutation.current||loading)return;
  ...
  try{
   ...
   const selectedWeek=await readWeek(selectedDate);
   setWeek(selectedWeek);
   const refreshed=selectedWeek.days.find(day=>day.date===selectedDate);if(refreshed)selectDay(refreshed);
 ...
 return <div className="sleep-dialog-overlay fixed inset-0 z-50 bg-stone-950/45 p-0 backdrop-blur-sm sm:p-5">
  <section role="dialog" aria-modal="true" aria-labelledby="sleep-dialog-title" className="sleep-dialog-panel w-full max-w-lg rounded-t-[2rem] bg-white p-6 shadow-2xl sm:rounded-[2rem] sm:p-7">
   <header className="flex items-start justify-between gap-4"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-indigo-100 text-indigo-700"><MoonStar size={21}/></span><div><p className="text-xs font-semibold text-indigo-600">{selectedDate&&selectedDate!==today?`Sono de ${formatSelectedDate(selectedDate)}`:"Sono de hoje"}</p><h2 id="sleep-dialog-title" className="text-xl font-bold">Quando você dormiu?</h2></div></div><button type="button" disabled={saving||deleting} onClick={onClose} aria-label="Fechar registro de sono" className="grid size-10 place-items-center rounded-xl bg-stone-100 text-stone-600 outline-none focus-visible:ring-4 focus-visible:ring-indigo-200"><X size={19}/></button></header>
   <div className="mt-5 flex items-end gap-2"><label className="min-w-0 flex-1 text-xs font-semibold text-stone-600">Data em que acordou<input type="date" max={today} value={selectedDate} disabled={saving||deleting||loading} onInput={event=>void chooseDate(event.currentTarget.value)} className="mt-2 block min-h-11 w-full min-w-0 rounded-xl border border-stone-200 bg-stone-50 px-3 text-base text-stone-900 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"/></label>{selectedDate!==today&&<button type="button" disabled={saving||deleting||loading} onClick={()=>void chooseDate(today)} className="min-h-11 rounded-xl bg-indigo-50 px-3 text-sm font-bold text-indigo-700">Hoje</button>}</div>
   {loading&&<p role="status" className="mt-5 text-sm text-stone-500">Carregando sono…</p>}
   {week&&<SleepWeekCalendar week={week} selectedDate={selectedDate} onSelect={selectDay} disabled={saving||deleting||loading}/>}
   {weekError&&<p className="mt-4 text-sm text-stone-500">{weekError}</p>}
   <form onSubmit={save} className="mt-6"><fieldset disabled={saving||deleting||loading||!selectedDay}><div className="grid grid-cols-2 gap-3"><TimeField label="Hora de dormir" value={bedtime} onChange={setBedtime}/><TimeField label="Hora de acordar" value={wakeTime} onChange={setWakeTime}/></div><div className="mt-4 rounded-2xl bg-indigo-50 p-4"><p className="text-xs font-semibold text-indigo-600">Duração aproximada</p><p className="mt-1 text-2xl font-bold text-indigo-950">{preview}</p></div>{error&&<p role="alert" className="mt-4 rounded-2xl bg-rose-50 p-4 text-sm text-rose-700">{error}</p>}<button disabled={saving||!selectedDate} className="sleep-dialog-save mt-5 min-h-14 w-full rounded-2xl px-5 font-bold text-white outline-none transition focus-visible:ring-4 focus-visible:ring-indigo-200 disabled:opacity-60">{saving?"Salvando...":selectedDay?.hasSleep?"Salvar alterações":selectedDate===today?"Salvar sono":"Salvar neste dia"}</button></fieldset></form>
   {selectedDay?.hasSleep&&!confirmingDelete&&<button type="button" disabled={saving||deleting||loading} onClick={()=>setConfirmingDelete(true)} className="mt-3 min-h-11 w-full rounded-xl text-sm font-semibold text-rose-700 outline-none hover:bg-rose-50 focus-visible:ring-4 focus-visible:ring-rose-200">Excluir sono deste dia</button>}
...
function SleepWeekCalendar({week,selectedDate,onSelect,disabled}:{week:SleepWeek;selectedDate:string;onSelect:(day:SleepWeekDay)=>void;disabled:boolean}){
 const today=getLocalToday();
 const selected=week.days.find(day=>day.date===selectedDate);
 return <section aria-label="Sono da semana selecionada" className="mt-5 rounded-2xl bg-indigo-50/70 p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-bold">{week.days.some(day=>day.isToday)?"Esta semana":`${formatCalendarDate(week.days[0].date)} – ${formatCalendarDate(week.days[6].date)}`}</h3><span className="text-right text-xs font-semibold text-indigo-700">{week.completedDays} dia{week.completedDays===1?"":"s"} · média {formatDuration(week.averageMinutes)}</span></div><div className="mt-3 grid grid-cols-7 gap-1">{week.days.map((day,index)=>{const future=day.date>today;return <button key={day.date} type="button" disabled={future||disabled} onClick={()=>onSelect(day)} aria-label={`${dayLabels[index]} ${formatCalendarDate(day.date)}${day.hasSleep?", sono registrado":""}${day.isToday?", hoje":""}${future?", indisponível":""}`} aria-pressed={selectedDate===day.date} className={`flex min-h-12 flex-col items-center justify-center rounded-xl text-[10px] font-bold outline-none transition focus-visible:ring-4 focus-visible:ring-indigo-200 disabled:cursor-not-allowed disabled:opacity-40 ${day.hasSleep?"bg-indigo-600 text-white":"bg-white/70 text-stone-500"} ${selectedDate===day.date?"ring-2 ring-indigo-400":""}`}><span>{dayLabels[index]}</span><span className="mt-1 grid h-4 place-items-center">{day.hasSleep?<Check size={14} strokeWidth={3}/>:new Date(`${day.date}T12:00:00`).getDate()}</span></button>})}</div><div className="mt-3 min-h-9 rounded-xl bg-white px-3 py-2 text-xs">{selected?.hasSleep?<p><b>{formatDuration(selected.durationMinutes??0)}</b> · {selected.bedtime} às {selected.wakeTime}{selected.metGoal&&<span className="text-indigo-600"> · meta alcançada</span>}</p>:<p className="text-stone-500">{selected?.isToday?"Hoje ainda não há sono registrado.":"Nenhum sono neste dia. Você pode registrar abaixo."}</p>}</div></section>
}

function TimeField({label,value,onChange}:{label:string;value:string;onChange:(value:string)=>void}){return <label className="min-w-0 text-sm font-bold text-stone-700">{label}<input required type="time" value={value} onInput={event=>onChange(event.currentTarget.value)} className="mt-2 block h-14 w-full min-w-0 rounded-2xl border border-stone-200 bg-stone-50 px-4 text-base outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"/></label>}
