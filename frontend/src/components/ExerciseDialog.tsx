...
export type ExerciseEntry={date:string;type:string;durationMinutes:number;distanceKm:number|null;caloriesBurned:number|null;paceSecondsPerKm:number|null;averageSpeedKmh:number|null;note:string};
type ExerciseWeekDay={date:string;isToday:boolean;hasExercise:boolean;type:string|null;durationMinutes:number|null;distanceKm:number|null;caloriesBurned:number|null;paceSecondsPerKm:number|null;averageSpeedKmh:number|null;note:string};
...
export type ExerciseWeek={startDate:string|null;endDate:string|null;completedDays:number;targetDays:number;totalMinutes:number;totalCalories:number;distanceByModality:ExerciseDistanceTotal[];days:ExerciseWeekDay[]};
...
export default function ExerciseDialog({open,onClose,onSaved,onDeletedToday,onWeekChanged,initialEntry}:ExerciseDialogProps){
 ...
 useEffect(()=>{
  ...
  setCaloriesBurned(initialEntry?.caloriesBurned?.toString()??"");
  setError("");
  setWeekError("");
 ...
 async function save(event:FormEvent){
  ...
  try{
   ...
   const response=await fetch("/api/health/exercise",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type,durationMinutes:totalMinutes,distanceKm:distanceTypes.includes(type)?distanceKm:null,caloriesBurned,note,exerciseDate:selectedDate,customActivity})});
 ...
 function selectDay(day:ExerciseWeekDay){
  ...
  setCaloriesBurned(day.caloriesBurned?.toString()??"");
  setNote(day.note??"");
  setError("");
  setConfirmingDelete(false);
 }
 ...
 return <div className="health-dialog-overlay fixed inset-0 z-50 bg-stone-950/45 p-0 backdrop-blur-sm sm:p-5">
  <section role="dialog" aria-modal="true" aria-labelledby="exercise-dialog-title" className="health-dialog-panel w-full max-w-lg rounded-t-[2rem] bg-white p-6 shadow-2xl sm:rounded-[2rem] sm:p-7">
   ...
   <form onSubmit={save} className="mt-6">
    ...
    <label className="mt-5 block text-sm font-bold text-stone-700">Calorias ativas <span className="font-normal text-stone-400">(opcional)</span><div className="mt-2 flex h-14 items-center rounded-2xl border border-stone-200 bg-stone-50 px-4 focus-within:border-orange-500 focus-within:ring-4 focus-within:ring-orange-100"><input type="number" min="1" max="10000" inputMode="numeric" value={caloriesBurned} onChange={event=>setCaloriesBurned(event.target.value)} placeholder="Ex.: 420" className="min-w-0 flex-1 bg-transparent text-base font-bold text-stone-900 outline-none"/><span className="text-sm font-semibold text-orange-500">kcal</span></div><span className="mt-2 block text-xs font-normal text-stone-400">Informe o valor exibido pelo seu relógio ou aplicativo.</span></label>
...
function getPerformancePreview(type:string,hours:string,minutes:string,distanceValue:string){const duration=(Number(hours)||0)*60+(Number(minutes)||0),distance=Number(distanceValue.replace(",","."));if(!duration||!distance)return "";if(type==="Corrida")return `Pace médio estimado · ${formatPace(Math.round(duration*60/distance))}`;if(type==="Ciclismo")return `Velocidade média estimada · ${(distance/(duration/60)).toLocaleString("pt-BR",{maximumFractionDigits:1})} km/h`;return ""}
